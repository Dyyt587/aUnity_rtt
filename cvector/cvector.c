...
int cvector_popback(const cvector cv, void *memb)
{
	...
	if ((cv->cv_tot_len >= (MIN_LEN << REDUSED_VAL)) && (cv->cv_len <= (cv->cv_tot_len >> REDUSED_VAL)))
	{
		void *pd_sav = cv->cv_pdata;
		cv->cv_tot_len >>= EXPANED_VAL;
		cv->cv_pdata = realloc(cv->cv_pdata, cv->cv_tot_len * cv->cv_size);
...
citerator cvector_end(const cvector cv)
{
 ...
 static void cvmemove_foreward(const cvector cv, void *from, void *to)
{
 ...
 static void cvmemove_backward(const cvector cv, void *from, void *to)
{
